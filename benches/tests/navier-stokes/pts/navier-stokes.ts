var solver: FluidField | null = null;
...
class Field {
    constructor(public readonly dens: number[], public readonly u: number[], public readonly v: number[]) {}
    // Just exposing the fields here rather than using accessors is a measurable win during display (maybe 5%)
    // but makes the code ugly.
    setDensity(x: number, y: number, d: number) {
        this.dens[(x + 1) + (y + 1) * rowSize] = d;
    }
    getDensity(x: number, y: number) {
         return this.dens[(x + 1) + (y + 1) * rowSize];
    }
    setVelocity(x: number, y: number, xv: number, yv: number) {
        this.u[(x + 1) + (y + 1) * rowSize] = xv;
        this.v[(x + 1) + (y + 1) * rowSize] = yv;
    }
    getXVelocity(x: number, y: number) {
         return this.u[(x + 1) + (y + 1) * rowSize];
    }
    getYVelocity(x: number, y: number) {
         return this.v[(x + 1) + (y + 1) * rowSize];
    }
    width() { return width; }
...
function diffuse(b: number, x: number[], x0: number[], dt: any)
{
    const a = 0;
...
function diffuse2(x: number[], x0: number[], y: number[], y0: number[], dt: any)
{
    const a = 0;
...
var uiCallback = function(_c: any) {};
...
class FluidField {
    ...
    setResolution(hRes: number, wRes: number)
    {
        var res = wRes * hRes;
