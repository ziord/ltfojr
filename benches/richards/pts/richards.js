...
class Scheduler {
    ...
    addTask(id, priority, queue, task) {
        ...
        $__getByIdOffset(this, "blocks", 0)[id] = this.currentTcb;
    }
    schedule() {
    ...
    release(id) {
        const tcb = $__getByIdOffset(this, "blocks", 0)[id];
        if (tcb === null)
            return tcb;
        tcb.markAsNotHeld();
        if ($__getByIdOffset(tcb, "priority", 0) > this.currentTcb.priority) {
            return tcb;
        }
        else {
    ...
    queue(packet) {
        var _a;
        const t = $__getByIdOffset(this, "blocks", 0)[packet.id];
        if (t === null)
            return t;
        this.queueCount++;
        packet.link = null;
...
class TaskControlBlock {
    ...
    run() {
        ...
        return $__getByIdOffset(this, "task", 1).run(packet);
    }
    checkPriorityAdd(task, packet) {
        if (this.queue === null) {
            this.queue = packet;
            this.markAsRunnable();
            if ($__getByIdOffset(this, "priority", 0) > $__getByIdOffset(task, "priority", 0))
                return this;
        }
        else {
    ...
    toString() {
        return "tcb { " + $__getByIdOffset(this, "task", 1) + "@" + this.state + " }";
    }
}
class IdleTask {
    ...
    run(packet) {
        if (packet === null) {
            ...
            for (let i = 0; i < DATA_SIZE; i++) {
                ...
                if (this.v2 > 26)
                    this.v2 = 1;
                $__getByIdOffset(packet, "a2", 0)[i] = this.v2;
            }
...
class HandlerTask {
    ...
    run(packet) {
        if (packet !== null) {
            if ($__getByIdOffset(packet, "kind", 1) === KIND_WORK) {
        ...
        if (this.v1 !== null) {
            ...
            if (count < DATA_SIZE) {
                if (this.v2 !== null) {
                    ...
                    v.a1 = $__getByIdOffset(this.v1, "a2", 0)[count];
                    this.v1.a1 = count + 1;
